...
public class Chapter1PerformanceController : MonoBehaviour
{
    ...
    public bool playStoryNarration = true;
    ...
    private void OnGUI()
    {
        ...
        if (showExplorationTimer && freeExplorationUnlocked && !policeSequenceStarted)
        {
            DrawExplorationTimer();
        }

        if (showWeddingQuestProgress && freeExplorationUnlocked && !policeSequenceStarted && !chapterCompleted)
        {
            DrawWeddingQuestProgress();
        }
    ...
    private void DrawWeddingQuestProgress()
    {
        float width = Mathf.Min(280f, Screen.width - 40f);
        float height = 178f;
        float x = Screen.width - width - 20f;
        float y = showExplorationTimer ? 108f : 20f;
        Rect box = new Rect(x, y, width, height);
        GUI.Box(box, GUIContent.none, timerBoxStyle);

        int wineTarget = Mathf.Max(1, wineTargetCount);
        int foodTarget = Mathf.Max(1, foodTargetCount);
        string danceProgress = danceFinished ? "已完成" : "未完成";
        string progress = "送酒  " + Mathf.Min(deliveredWineCount, wineTarget) + " / " + wineTarget
            + "\n送食物  " + Mathf.Min(sharedFoodCount, foodTarget) + " / " + foodTarget
            + "\n跳舞  " + danceProgress;
        string requirement = AreWeddingTasksComplete() ? "任務完成，準備進入劇情" : "完成任務才可以繼續";

        GUI.Label(new Rect(x + 16f, y + 10f, width - 32f, 24f), "任務完成度", timerTitleStyle);
        GUI.Label(new Rect(x + 16f, y + 38f, width - 32f, 88f), progress, hudBodyStyle);
        GUI.Label(new Rect(x + 16f, y + 132f, width - 32f, 34f), requirement, timerTitleStyle);
    }
