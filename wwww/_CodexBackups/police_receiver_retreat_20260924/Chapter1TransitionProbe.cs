...
{
    static readonly string Folder = Path.GetFullPath("_CodexBackups/police_receiver_retreat_20260924");
    ...
    [Serializable] class Actor { public string name; public Vector3 position; public bool human; public float rigHeight; public string[] bones; }
    ...
    static void WriteState(string filename)
    {
        ...
        foreach (var dancer in Actors())
        {
            ...
            actors.Add(new Actor { name = dancer.name, position = dancer.transform.position, human = animator != null && animator.isHuman, rigHeight = rig != null ? rig.Height : 0f, bones = bones.ToArray() });
        }
