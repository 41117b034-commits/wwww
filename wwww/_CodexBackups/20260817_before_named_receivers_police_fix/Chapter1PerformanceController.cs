...
public class Chapter1PerformanceController : MonoBehaviour
{
    ...
    public bool playStoryNarration = false;
    ...
    public float explorationDurationSeconds = 30f;
    ...
    private bool movementLockedForPendingTasks;
    ...
    public void BeginChapter()
    {
        ...
        movementLockedForPendingTasks = false;
    ...
    private void UpdateExplorationTimer()
    {
        ...
        {
            movementLockedForPendingTasks = !AreWeddingTasksComplete();
            if (movementLockedForPendingTasks)
            {
                SetPlayerControl(false);
            }
    ...
    private bool ShouldShowCenterInteractionMenu()
    {
        ...
        return movementLockedForPendingTasks || !requireFireDistanceForCenterMenu || IsPlayerNearFireCenter();
    }
    ...
    public void DeliverWine(string npcName, Transform receiverTarget = null)
    {
        ...
        receiverTarget = ResolveUnusedInteractionTarget(true, receiverTarget);
    ...
    public void ShareFood(string npcName, Transform receiverTarget = null)
    {
        ...
        receiverTarget = ResolveUnusedInteractionTarget(false, receiverTarget);
    ...
    private Transform FindNextInteractionTarget(bool isWine)
    {
        ...
        for (int offset = 0; offset < count; offset++)
        {
            ...
            if (target != null && !completedTargets.Contains(target.transform.GetInstanceID()))
            {
        ...
        existingTarget = FindExistingInteractionTarget(isWine, false);
        if (existingTarget != null)
        {
            return existingTarget;
        }

        return FindUnusedWeddingGuestTarget(completedTargets);
    }
    ...
    private Transform ResolveUnusedInteractionTarget(bool isWine, Transform candidate)
    {
        ...
        return FindNextInteractionTarget(isWine);
    }
    ...
    private Transform FindExistingInteractionTarget(bool isWine, bool requireActive)
    {
        ...
        for (int i = 0; i < interactables.Length; i++)
        {
            ...
            if (completedTargets.Contains(interactable.transform.GetInstanceID()))
            {
                continue;
            }
    ...
    private Transform FindUnusedWeddingGuestTarget(HashSet<int> completedTargets)
    {
        Transform center = danceCenter != null ? danceCenter : transform;
        ...
        for (int i = 0; i < animators.Length; i++)
        {
            ...
            float distance = (candidate.position - center.position).sqrMagnitude;
            if (distance < bestDistance)
            {
                bestDistance = distance;
    ...
    private string GetWeddingQuestMissionText()
    {
        ...
        string continueHint = movementLockedForPendingTasks ? "｜完成任務才可以繼續" : "";
    ...
    {
        bool shouldEnable = enabled && !movementLockedForPendingTasks;
    ...
    {
        float narrationSeconds = PlayStoryNarrationForLine(speaker, line);
        if (narrationSeconds > 0f)
        {
            seconds = Mathf.Max(seconds, narrationSeconds + 0.15f);
        }
    ...
    private float PlayStoryNarrationForLine(string speaker, string line)
    {
        if (!playStoryNarration || (speaker != "旁白" && speaker != "字幕"))
        {
            return 0f;
        }

        string clipName = GetStoryNarrationClipName(line);
        if (string.IsNullOrEmpty(clipName))
        {
            return 0f;
        }

        AudioClip clip = Resources.Load<AudioClip>("Chapter1Voice/" + clipName);
        if (clip == null)
        {
            Debug.LogWarning("[Chapter1] Missing story narration clip: " + clipName);
            return 0f;
        }
    ...
    private string GetStoryNarrationClipName(string line)
    {
        if (line.StartsWith("鼓聲突然慢了下來")) return "01_police_approach";
        if (line.StartsWith("兩名日警突然從山路闖入")) return "02_police_arrival";
        if (line.StartsWith("警察推倒酒杯")) return "03_cup_and_shove";
        if (line.StartsWith("另一名警察把目光轉向")) return "04_woman_approach";
        if (line.StartsWith("一名警察試圖騷擾")) return "05_woman_harassment";
        if (line.StartsWith("族人望著日警下山")) return "06_closing_memory";
        if (line.StartsWith("警察冷笑著抬手")) return "07_cup_swept";
        if (line.StartsWith("一名族人上前質問")) return "08_villager_shoved";
        if (line.StartsWith("你沒有上前")) return "09_watch_drag";
        if (line.StartsWith("木屋門關上後")) return "10_hut_cry";
        if (line.StartsWith("你沉默地站在原地")) return "11_watch_alternate";
        if (line.StartsWith("幾名族人憤而衝上前")) return "12_villagers_intervene";
        if (line.StartsWith("砰")) return "13_gunshot";
        if (line.StartsWith("兩名警察整理衣服")) return "14_police_leave";
        return "";
    }
