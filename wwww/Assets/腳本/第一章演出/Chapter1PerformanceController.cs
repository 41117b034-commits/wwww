...
public class Chapter1PerformanceController : MonoBehaviour
{
    ...
    public float minimumPoliceExitSeconds = 8f;
    ...
    private Renderer[] endingHiddenHandRenderers;
    private bool[] endingHiddenHandRendererStates;
    ...
    private void Start()
    {
        ...
        PrepareDeliveryTaskNPCs();
        StartCoroutine(RepairWeddingPickupPropsAfterSceneReady());
    ...
    private void OnGUI()
    {
        ...
        if (waitingForChoice && choiceUI == null)
        {
            ...
            float height = 178f;
            float bottomClearance = 122f;
            float choiceY = Mathf.Min(
                Screen.height * 0.61f,
                Screen.height - height - bottomClearance);
            Rect box = new Rect(
                (Screen.width - width) * 0.5f,
                Mathf.Max(20f, choiceY),
                width,
                height);
    ...
    private IEnumerator RepairWeddingPickupPropsAfterSceneReady()
    {
        if (!IsNewPoliceScene())
        {
            yield break;
    ...
    private void FlattenFoodPickup(Transform food)
    {
        ...
        for (int i = 0; i < filters.Length; i++)
        {
            ...
            if (reference == null || volume > largestVolume)
            {
                reference = filters[i];
        ...
        Vector3 meshSize = reference.sharedMesh.bounds.size;
        Vector3 localThinAxis = meshSize.x <= meshSize.y && meshSize.x <= meshSize.z
            ? Vector3.right
            : (meshSize.y <= meshSize.z ? Vector3.up : Vector3.forward);
        Vector3 worldThinAxis = reference.transform.TransformDirection(localThinAxis).normalized;
        if (Vector3.Dot(worldThinAxis, Vector3.up) < 0f)
        {
            worldThinAxis = -worldThinAxis;
        }

        food.rotation = Quaternion.FromToRotation(worldThinAxis, Vector3.up)
            * food.rotation;
        Physics.SyncTransforms();
    }

    private void SnapPickupPropToGround(Transform prop, float clearance)
    {
        if (prop == null || !TryGetVisibleBounds(prop, out Bounds bounds))
        {
            return;
        }

        Physics.SyncTransforms();
        if (!TryGetPickupGroundY(prop, bounds, out float groundY))
        {
            return;
        }

        Vector3 position = prop.position;
        position.y += groundY + Mathf.Max(0f, clearance) - bounds.min.y;
        prop.position = position;
        Physics.SyncTransforms();
    }

    private bool TryGetPickupGroundY(
        Transform prop,
        Bounds bounds,
        out float groundY)
    {
        float probeHeight = Mathf.Max(bounds.max.y + 2f, prop.position.y + 2f);
        Vector3 origin = new Vector3(bounds.center.x, probeHeight, bounds.center.z);
        ...
        float highestAllowedSurface = prop.position.y
            + Mathf.Max(1.5f, bounds.extents.y + 0.5f);
        for (int i = 0; i < hits.Length; i++)
        {
            RaycastHit hit = hits[i];
            if (hit.collider == null
                || hit.normal.y < 0.45f
                || hit.point.y > highestAllowedSurface)
            {
                continue;
            }

            Transform hitTransform = hit.collider.transform;
            if (hitTransform == prop
                || hitTransform.IsChildOf(prop)
                || prop.IsChildOf(hitTransform)
        ...
        for (int i = 0; i < terrains.Length; i++)
        {
            ...
            Vector3 terrainPosition = terrain.transform.position;
            Vector3 terrainSize = terrain.terrainData.size;
            if (origin.x < terrainPosition.x
                || origin.x > terrainPosition.x + terrainSize.x
                || origin.z < terrainPosition.z
                || origin.z > terrainPosition.z + terrainSize.z)
            {
                continue;
            }

            groundY = terrain.SampleHeight(origin) + terrainPosition.y;
            return true;
        }
    ...
    private void EnsureWeddingCrowdDancers()
    {
        ...
        Transform facingCenter = circleCenter;
        ...
        for (int i = 0; i < animators.Length; i++)
        {
            ...
            // Generic rigs keep their authored pose so their meshes stay intact.
            bool hasSafeHumanoidRig = animator.avatar != null
                && animator.avatar.isValid
                && animator.isHuman;
            dancer.enableProceduralStepping = hasSafeHumanoidRig;
            dancer.upperLegLiftDegrees = 9f;
            dancer.kneeBendDegrees = 16f;
            dancer.pelvisBobHeightRatio = 0.007f;
            dancer.pelvisWeightShiftRatio = 0.004f;
            ...
            if (limbDance == null)
            {
                limbDance = actorRoot.gameObject.AddComponent<Chapter1WeddingLimbDance>();
            }
    ...
    private IEnumerator PoliceSequenceRoutine()
    {
        ...
        PrepareIncidentVictimForPoliceSequence();
        EnsureCinematicActorGrounding(primaryPoliceActor, true);
        EnsureCinematicActorGrounding(secondaryPoliceActor, true);
        EnsureCinematicActorGrounding(femaleVillagerActor, true);
        ...
        if (femaleVillagerActor != null && harassingPolice != null)
        {
            ...
            float harassmentDistance = GetIncidentPersonalSpace(
                ...
                0.32f,
                0.58f);
            harassmentDistance = Mathf.Clamp(harassmentDistance, 0.58f, 1.05f);
            ...
            EnsureCinematicActorGrounding(femaleVillagerActor, true);
            StageHarassmentPair(harassingPolice, femaleVillagerActor);
    ...
    private IEnumerator CinematicFrameIncidentActors(
        ...
        float seconds,
        bool keepOccludersHidden = false,
        float framingDistanceScale = 1f)
    {
        RestoreIncidentShotOccluders();
        ...
        if (incidentCameraPoseLock == null)
        {
            ...
            incidentHiddenHandRenderers = HidePlayerHandRenderers(
                out incidentHiddenHandRendererStates);
        }
        ...
        float framingDistance = Mathf.Max(
            3.2f,
            stableFrameDistance * Mathf.Max(0.55f, framingDistanceScale));
        Vector3 preferredDirection = (
            outward + side * incidentCameraSideRatio).normalized;
        Vector3 desiredPosition = GetDirectIncidentCameraPosition(
            focus,
            firstActor,
            secondActor,
            preferredDirection,
        ...
        HideIncidentShotOccluders(
            ...
            firstActor,
            secondActor,
            Mathf.Max(12f, verticalExtent * 8f));
        ...
        if (!keepOccludersHidden)
        {
            RestoreIncidentShotOccluders();
        }
    }

    private IEnumerator CinematicFrameIncidentActorsFromWitness(
        Transform firstActor,
        Transform secondActor,
        float seconds,
        bool keepOccludersHidden)
    {
        RestoreIncidentShotOccluders();
        ...
        if (incidentCameraView == null || firstActor == null || !hasPoliceWitnessViewPose)
        {
            yield return CinematicFrameIncidentActors(
                firstActor,
                secondActor,
        ...
        GetStableIncidentFrame(
            ...
            out float ignoredDistance);
        // Aim below the grip so both upper bodies and the grabbing arm remain
        // visible above the choice panel while the witness position stays fixed.
        focus.y -= verticalExtent * 0.68f;
        Vector3 witnessPosition = policeWitnessViewPosition;
        Vector3 lookDirection = focus - witnessPosition;
        Quaternion witnessRotation = lookDirection.sqrMagnitude > 0.01f
            ? Quaternion.LookRotation(lookDirection.normalized, Vector3.up)
        ...
        while (elapsed < duration)
        {
            ...
            SetCinematicCameraPose(
                incidentCameraView,
                incidentCameraPoseLock,
                Vector3.Lerp(startPosition, witnessPosition, progress),
                Quaternion.Slerp(startRotation, witnessRotation, progress));
            yield return null;
        }
    ...
    private Vector3 GetDirectIncidentCameraPosition(
        Vector3 focus,
        Transform firstActor,
        Transform secondActor,
        ...
        float verticalExtent)
    {
        ...
        if (viewingDirection.sqrMagnitude < 0.01f)
        {
            viewingDirection = Vector3.back;
        }

        viewingDirection.Normalize();
        float safeDistance = Mathf.Max(2.2f, distance);
        float cameraLift = Mathf.Clamp(verticalExtent * 0.12f, 0.12f, 0.42f);
        float[] yawOffsets = { 0f, -18f, 18f, -34f, 34f, -52f, 52f, -68f, 68f };
        float[] distanceScales = { 1f, 1.14f };
        Vector3 fallback = focus + viewingDirection * safeDistance + Vector3.up * cameraLift;
        ...
        for (int distanceIndex = 0; distanceIndex < distanceScales.Length; distanceIndex++)
        {
            for (int yawIndex = 0; yawIndex < yawOffsets.Length; yawIndex++)
            {
                Vector3 direction = Quaternion.AngleAxis(
                    yawOffsets[yawIndex],
                    Vector3.up) * viewingDirection;
                Vector3 candidate = focus
                    + direction * (safeDistance * distanceScales[distanceIndex])
                    + Vector3.up * cameraLift;
                if (IsIncidentCameraPositionClear(
                    candidate,
                    focus,
                    firstActor,
                    secondActor,
                    Mathf.Max(0.8f, verticalExtent)))
                {
                    return candidate;
                }
            }
        ...
        out float maximumY)
    {
        ...
        Animator humanoid = actor.GetComponentInChildren<Animator>(true);
        if (humanoid != null && humanoid.isHuman && humanoid.avatar != null && humanoid.avatar.isValid)
        {
            Transform head = humanoid.GetBoneTransform(HumanBodyBones.Head);
            Transform hips = humanoid.GetBoneTransform(HumanBodyBones.Hips);
            Transform leftFoot = humanoid.GetBoneTransform(HumanBodyBones.LeftFoot);
            Transform rightFoot = humanoid.GetBoneTransform(HumanBodyBones.RightFoot);
            if (head != null && hips != null)
            {
                float feetY = actor.position.y;
                bool hasFoot = false;
                if (leftFoot != null)
                {
                ...
                if (rightFoot != null)
                {
                    feetY = hasFoot ? Mathf.Min(feetY, rightFoot.position.y) : rightFoot.position.y;
                    hasFoot = true;
                }

                float torsoLength = Mathf.Max(0.15f, Vector3.Distance(head.position, hips.position));
                float estimatedHeadTop = head.position.y + torsoLength * 0.18f;
                float bodyHeight = estimatedHeadTop - feetY;
                if (bodyHeight > 0.65f && bodyHeight < 4.5f)
                {
                    minimumY = feetY;
                    maximumY = estimatedHeadTop;
                    return true;
                }
            }
        }
    ...
    private float GetActorStandingHeight(Transform actor)
    {
    ...
    private void HideIncidentShotOccluders(
        Vector3 cameraPosition,
        Vector3 focus,
        Transform firstActor,
        Transform secondActor,
        float maximumDimension)
    {
        ...
        Transform player = GetDancePlayerRoot();
        Ray viewRay = new Ray(cameraPosition, toFocus / distance);
        float safeMaximumDimension = Mathf.Max(4f, maximumDimension);
        for (int i = 0; i < incidentOcclusionRenderers.Length; i++)
        {
            Renderer renderer = incidentOcclusionRenderers[i];
            if (renderer == null
                ...
                || renderer is LineRenderer
                || renderer is TrailRenderer
                || IsIncidentActorTransform(renderer.transform, firstActor, secondActor)
                || (player != null
                    && (renderer.transform == player
                        || renderer.transform.IsChildOf(player))))
            {
            ...
            if (size.sqrMagnitude < 0.0001f
                || largestDimension > safeMaximumDimension)
            {
                continue;
            }

            rendererBounds.Expand(0.12f);
            if (rendererBounds.Contains(cameraPosition)
                || (rendererBounds.IntersectRay(viewRay, out float entryDistance)
                    && entryDistance > 0.05f
                    && entryDistance < distance - 0.08f))
            {
                renderer.enabled = false;
                incidentShotHiddenRenderers.Add(renderer);
            }
        }
    }

    private void RestoreIncidentShotOccluders()
    {
        for (int i = 0; i < incidentShotHiddenRenderers.Count; i++)
        {
            Renderer renderer = incidentShotHiddenRenderers[i];
            if (renderer != null)
            {
        ...
        incidentShotHiddenRenderers.Clear();
    }
    ...
    private IEnumerator CinematicPoliceFrontRevealAndFirstLine()
    {
        ...
        float policeHeight = hasPrimaryPoliceBounds
            ? Mathf.Max(1f, primaryPoliceBounds.size.y)
            : GetActorStandingHeight(primaryPoliceActor);
        float sideRatio = policeFrontRevealSideOffset
            / Mathf.Max(0.1f, revealDistance);
        Vector3 desiredPosition = GetFrontSpeakerCameraPosition(
            primaryPoliceActor,
            focusPoint,
            (policeForward + policeRight * sideRatio).normalized,
            revealDistance,
            policeHeight);
        HideIncidentShotOccluders(
            desiredPosition,
    ...
    private IEnumerator PlayCinematicSpeakerLine(
        Transform speakerActor,
        string speaker,
        string line,
        AudioClip voiceClip,
        ...
        float volumeScale = 1f)
    {
        RestoreIncidentShotOccluders();
        Transform view = GetPlayerViewTransform();
        if (!usePoliceCinematicCamera || speakerActor == null || view == null)
        {
            yield return PlayPoliceVoicedLine(
                speaker,
                line,
        ...
        Vector3 originalPosition = view.position;
        Quaternion originalRotation = view.rotation;
        Vector3 originalLocalPosition = view.localPosition;
        Quaternion originalLocalRotation = view.localRotation;

        bool usesIncidentLock = view == incidentCameraView && incidentCameraPoseLock != null;
        bool usesEndingLock = view == endingCameraView && endingCameraPoseLock != null;
        Chapter1CinematicCameraPoseLock poseLock = usesIncidentLock
            ? incidentCameraPoseLock
            : (usesEndingLock ? endingCameraPoseLock : null);
        List<Behaviour> trackedPoseDrivers = null;
        Renderer[] hiddenHandRenderers = null;
        bool[] hiddenHandRendererStates = null;
        bool ownsCameraLock = poseLock == null;
        if (ownsCameraLock)
        {
            trackedPoseDrivers = DisableCameraTrackedPoseDrivers(view);
            poseLock = BeginCinematicCameraPoseLock(view);
            hiddenHandRenderers = HidePlayerHandRenderers(out hiddenHandRendererStates);
        }

        float minimumY;
        float maximumY;
        float actorHeight;
        if (TryGetStableActorVerticalSpan(speakerActor, out minimumY, out maximumY))
        {
            actorHeight = Mathf.Max(1f, maximumY - minimumY);
        }
        else
        {
            actorHeight = GetActorStandingHeight(speakerActor);
            minimumY = speakerActor.position.y;
            maximumY = minimumY + actorHeight;
        }

        Vector3 actorForward = Vector3.ProjectOnPlane(speakerActor.forward, Vector3.up);
        if (actorForward.sqrMagnitude < 0.01f)
        {
            actorForward = Vector3.ProjectOnPlane(view.position - speakerActor.position, Vector3.up);
        }
        if (actorForward.sqrMagnitude < 0.01f)
        {
        ...
        Vector3 actorRight = Vector3.Cross(Vector3.up, actorForward).normalized;
        ...
        HideIncidentShotOccluders(
            ...
            speakerActor,
            null,
            Mathf.Max(12f, actorHeight * 5f));
        Quaternion desiredRotation = Quaternion.LookRotation(
            (focusPoint - desiredPosition).normalized,
            Vector3.up);

        float moveDuration = Mathf.Max(0.25f, policeCameraPanSeconds * 0.65f);
        float elapsed = 0f;
        while (elapsed < moveDuration)
        {
            elapsed += Time.deltaTime;
            ...
            SetCinematicCameraPose(
                view,
                poseLock,
                Vector3.Lerp(originalPosition, desiredPosition, t),
                Quaternion.Slerp(originalRotation, desiredRotation, t));
            yield return null;
        }

        SetCinematicCameraPose(view, poseLock, desiredPosition, desiredRotation);
        yield return PlayPoliceVoicedLine(
            speaker,
            line,
            voiceClip,
        ...
        Vector3 closePosition = view.position;
        Quaternion closeRotation = view.rotation;
        float returnDuration = Mathf.Max(0.25f, policeTrackingReturnSeconds * 0.55f);
        elapsed = 0f;
        while (elapsed < returnDuration)
        {
            elapsed += Time.deltaTime;
            ...
            SetCinematicCameraPose(
                view,
                poseLock,
                Vector3.Lerp(closePosition, originalPosition, t),
                Quaternion.Slerp(closeRotation, originalRotation, t));
            yield return null;
        }

        SetCinematicCameraPose(view, poseLock, originalPosition, originalRotation);
        if (ownsCameraLock)
        {
            EndCinematicCameraPoseLock(poseLock);
            view.localPosition = originalLocalPosition;
            view.localRotation = originalLocalRotation;
            RestoreCameraTrackedPoseDrivers(trackedPoseDrivers);
            RestorePlayerHandRenderers(hiddenHandRenderers, hiddenHandRendererStates);
        }
        RestoreIncidentShotOccluders();
    }

    private Vector3 GetFrontSpeakerCameraPosition(
        Transform speakerActor,
        Vector3 focusPoint,
        ...
        float actorHeight)
    {
        ...
        float lift = Mathf.Clamp(actorHeight * 0.04f, 0.06f, 0.16f);
        float[] yawOffsets = { 0f, -14f, 14f, -26f, 26f, -38f, 38f };
        float[] distanceScales = { 1f, 1.18f, 1.36f };
        Vector3 fallback = focusPoint
            + frontDirection * (distance * distanceScales[distanceScales.Length - 1])
            + Vector3.up * lift;
        ...
        for (int distanceIndex = 0; distanceIndex < distanceScales.Length; distanceIndex++)
        {
            for (int yawIndex = 0; yawIndex < yawOffsets.Length; yawIndex++)
            {
                Vector3 direction = Quaternion.AngleAxis(
                    yawOffsets[yawIndex],
                    Vector3.up) * frontDirection;
                Vector3 candidate = focusPoint
                    + direction * (distance * distanceScales[distanceIndex])
                    + Vector3.up * lift;
                if (IsIncidentCameraPositionClear(
                    candidate,
                    focusPoint,
                    speakerActor,
                    null,
                    Mathf.Max(0.8f, actorHeight * 0.5f)))
                {
                    return candidate;
                }
            }
    ...
    private IEnumerator AnimatePoliceEntranceFallback()
    {
        ...
        float stableEntranceHeight = Mathf.Max(
            GetActorStandingHeight(firstPolice),
            GetActorStandingHeight(secondPolice));
        float stableEntranceDistance = Mathf.Clamp(
            stableEntranceHeight * 2.45f + policePairSpacing * 0.55f,
            6.8f,
            10.5f);
        Vector3 entranceProbeCenter = (startPosition + endPosition) * 0.5f;
        Vector3 entranceProbeFocus = entranceProbeCenter
            + Vector3.up * stableEntranceHeight * 0.56f;
        Vector3 requestedEntranceDirection = (
            -normalizedPath + sideDirection * 0.28f).normalized;
        Vector3 clearEntranceProbe = GetDirectIncidentCameraPosition(
            entranceProbeFocus,
            firstPolice,
            secondPolice,
            requestedEntranceDirection,
            stableEntranceDistance,
            stableEntranceHeight * 0.5f);
        Vector3 stableEntranceViewDirection = Vector3.ProjectOnPlane(
            clearEntranceProbe - entranceProbeFocus,
            Vector3.up).normalized;
        if (stableEntranceViewDirection.sqrMagnitude < 0.01f)
        {
            stableEntranceViewDirection = requestedEntranceDirection;
        }
        Vector3 initialEntranceCenter = secondPolice != null
            ? (firstStart + secondStart) * 0.5f
            : firstStart;
        Vector3 smoothedVisibleLookPoint = initialEntranceCenter
            + Vector3.up * stableEntranceHeight * 0.56f;
        Vector3 smoothedVisibleCameraPosition = smoothedVisibleLookPoint
            + stableEntranceViewDirection * stableEntranceDistance;
        smoothedVisibleCameraPosition.y = smoothedVisibleLookPoint.y
            + stableEntranceHeight * 0.16f;
        ...
        while (elapsed < totalDuration)
        {
            ...
            Vector3 firstFramePosition =
                firstBase + sideDirection * firstDrift;
            firstFramePosition.y = firstPolice.position.y;
            firstPolice.position = firstFramePosition;
            ...
            if (secondPolice != null)
            {
                ...
                Vector3 secondFramePosition =
                    secondBase - sideDirection * secondDrift;
                secondFramePosition.y = secondPolice.position.y;
                secondPolice.position = secondFramePosition;
            }
                ...
                && playerView != null)
            {
                ...
                {
                    // Use the actor roots and one cached body height. Animated
                    // skinned bounds change every frame and made the old camera
                    // visibly jump while the officers walked.
                    lookPoint = new Vector3(
                        policeCenter.x,
                        policeCenter.y + stableEntranceHeight * 0.56f,
                        policeCenter.z);
                    ...
                    desiredCameraPosition.y =
                        lookPoint.y + stableEntranceHeight * 0.16f;

                    float cameraFollow = 1f - Mathf.Exp(-5f * Time.deltaTime);
                    smoothedVisibleCameraPosition = Vector3.Lerp(
                        smoothedVisibleCameraPosition,
                        desiredCameraPosition,
                        cameraFollow);
                    smoothedVisibleLookPoint = Vector3.Lerp(
                        smoothedVisibleLookPoint,
                        lookPoint,
                        cameraFollow);

                    Vector3 lookDirection = smoothedVisibleLookPoint - smoothedVisibleCameraPosition;
                    ...
                    SetCinematicCameraPose(
                        playerView,
                        poseLock,
                        Vector3.Lerp(originalViewPosition, smoothedVisibleCameraPosition, blend),
        ...
        SetHorizontalPosition(firstPolice, firstEnd);
        ...
        if (secondPolice != null)
        {
            SetHorizontalPosition(secondPolice, secondEnd);
        ...
        while (elapsed < duration)
        {
            ...
            if (incidentCameraView != null && incidentCameraPoseLock != null)
            {
                if (elapsed >= nextCameraRefresh)
                {
                    ...
                    Vector3 desiredCameraPosition = GetDirectIncidentCameraPosition(
                        desiredLookPoint,
                        police,
                        victim,
                        preferredDirection,
    ...
    private void StageHarassmentPair(Transform police, Transform victim)
    {
        if (police == null || victim == null)
        {
            return;
        ...
        if (outward.sqrMagnitude < 0.01f)
        {
            outward = Vector3.ProjectOnPlane(victim.forward, Vector3.up);
        }
        if (outward.sqrMagnitude < 0.01f)
        {
        ...
        float sideSpacing = GetNaturalGrabRootSpacing(police, victim);
        Vector3 policePosition = victim.position
            - right * sideSpacing
            - outward * Mathf.Clamp(sideSpacing * 0.12f, 0.04f, 0.08f);
        SetHorizontalPosition(police, policePosition);
    ...
    private float GetNaturalGrabRootSpacing(Transform police, Transform victim)
    {
        float policeHeight = GetActorStandingHeight(police);
        float victimHeight = GetActorStandingHeight(victim);
        float shorterHeight = Mathf.Min(policeHeight, victimHeight);
        return Mathf.Clamp(shorterHeight * 0.20f, 0.46f, 0.74f);
    }

    private void StageHarassmentPairForWitness(
        Transform police,
        Transform victim)
    {
        if (police == null || victim == null)
        ...
        {
            viewDirection = policeWitnessViewRotation * Vector3.forward;
            viewDirection.y = 0f;
        }
        if (viewDirection.sqrMagnitude < 0.01f)
        {
        ...
        Vector3 victimSideDirection)
    {
        ...
        if (policeAnimator == null || victimAnimator == null)
        {
            return;
        }
        ...
        Transform policeHead = Chapter1WeddingRigBones.Resolve(
            policeAnimator,
            HumanBodyBones.Head,
            "Head");
        Transform victimHead = Chapter1WeddingRigBones.Resolve(
            victimAnimator,
            HumanBodyBones.Head,
            "Head");
        if (policeHead == null || victimHead == null)
        {
            return;
        }

        Vector3 sideDirection = victimSideDirection;
        sideDirection.y = 0f;
        if (sideDirection.sqrMagnitude < 0.01f)
        {
            sideDirection = Vector3.right;
        ...
        {
            police.position += Vector3.ClampMagnitude(correction, 30f);
            Physics.SyncTransforms();
            policeAnimator.Update(0f);
        }
    }

    private void PrepareIncidentVictimForPoliceSequence()
    {
        ...
        Animator victimAnimator = femaleVillagerActor.GetComponentInChildren<Animator>(true);
        Transform victimRoot = GetWeddingDancerActorRoot(victimAnimator);
        if (victimRoot == null)
        {
            victimRoot = femaleVillagerActor;
        }
        femaleVillagerActor = victimRoot;
        ...
        for (int i = weddingCrowdDancers.Count - 1; i >= 0; i--)
        {
            ...
            Transform dancerRoot = dancer.transform;
            bool sameActor = dancerRoot == victimRoot
                || dancerRoot.IsChildOf(victimRoot)
                || victimRoot.IsChildOf(dancerRoot);
            if (!sameActor)
            {
                continue;
            }
            ...
            dancer.StopDancing();
            dancer.SetCanCircleDance(false);
            dancer.enabled = false;
            weddingCrowdDancers.RemoveAt(i);
        }

        Chapter1CircleDancer[] victimDancers =
            victimRoot.GetComponentsInChildren<Chapter1CircleDancer>(true);
        for (int i = 0; i < victimDancers.Length; i++)
        {
            victimDancers[i].StopDancing();
            victimDancers[i].SetCanCircleDance(false);
            victimDancers[i].enabled = false;
        }

        Chapter1HandHoldIK[] handHolding =
            victimRoot.GetComponentsInChildren<Chapter1HandHoldIK>(true);
        for (int i = 0; i < handHolding.Length; i++)
        {
            handHolding[i].enabled = false;
        }

        Chapter1FaceFire[] faceControllers =
            victimRoot.GetComponentsInChildren<Chapter1FaceFire>(true);
        for (int i = 0; i < faceControllers.Length; i++)
        {
            faceControllers[i].enabled = false;
        }

        Chapter1WeddingFaceCenter[] exactFacing =
            victimRoot.GetComponentsInChildren<Chapter1WeddingFaceCenter>(true);
        for (int i = 0; i < exactFacing.Length; i++)
        {
            exactFacing[i].enabled = false;
        }

        Chapter1WeddingLimbDance[] limbDance =
            victimRoot.GetComponentsInChildren<Chapter1WeddingLimbDance>(true);
        for (int i = 0; i < limbDance.Length; i++)
        {
            limbDance[i].enabled = false;
        }

        DetachActorFromDancePivot(victimRoot);
        Vector3 center = GetFireCenterPosition();
        Vector3 outward = victimRoot.position - center;
        outward.y = 0f;
        if (outward.sqrMagnitude < 0.01f && hasPoliceWitnessViewPose)
        {
            outward = Vector3.ProjectOnPlane(
                policeWitnessViewRotation * Vector3.right,
                Vector3.up);
        }
        if (outward.sqrMagnitude < 0.01f)
        {
        ...
        EnsureCinematicActorGrounding(victimRoot, true);
        Debug.Log("[Chapter1 Incident] Female actor removed from the dance circle and grounded.");
    }

    private Vector3 GetIncidentRightCornerPosition(Vector3 start)
    {
        Vector3 screenRight = hasPoliceWitnessViewPose
            ? policeWitnessViewRotation * Vector3.right
            : (GetPlayerViewTransform() != null
                ? GetPlayerViewTransform().right
                : Vector3.right);
        screenRight.y = 0f;
        if (screenRight.sqrMagnitude < 0.01f)
        {
    ...
    private void HoldPoliceWitnessViewTowardsActors(
        Transform firstActor,
        Transform secondActor)
    {
        if (!hasPoliceWitnessViewPose)
        ...
        GetStableIncidentFrame(
            ...
            out float verticalExtent,
            out float ignoredDistance);
        focus.y -= verticalExtent * 0.52f;
        HoldPoliceWitnessViewTowardsPoint(focus);
    }

    private void HoldPoliceWitnessViewTowardsPoint(Vector3 focus)
    {
        if (!hasPoliceWitnessViewPose)
        {
            CapturePoliceWitnessViewPose();
        ...
        Vector3 lookDirection = focus - policeWitnessViewPosition;
    ...
    private IEnumerator DragVictimWithFixedWitnessCameraRoutine(
        Transform police,
        Transform victim,
        Vector3 victimDestination,
        float seconds)
    {
        if (police == null || victim == null)
        {
        ...
        if (travelDirection.sqrMagnitude < 0.01f)
        {
            travelDirection = Vector3.ProjectOnPlane(police.forward, Vector3.up);
        }
        if (travelDirection.sqrMagnitude < 0.01f)
        {
        ...
        float seconds)
    {
        ...
        Vector3 side = Vector3.Cross(Vector3.up, travelDirection).normalized;
        float pairOffset = Mathf.Clamp(
            GetActorStandingHeight(victim) * 0.23f,
            0.40f,
            0.58f);
        Vector3 victimTarget = hutPoint - travelDirection * 0.12f;
        // Keep the victim on the officer's right-hand side for the whole drag.
        // Both roots travel as one pair so the grabbing hand cannot drift away.
        Vector3 policeTarget = victimTarget
            - side * pairOffset
            - travelDirection * 0.08f;
        Quaternion policeRotation = Quaternion.LookRotation(travelDirection, Vector3.up);
        Quaternion victimRotation = policeRotation;
        ...
        Vector3 clearDragCameraPosition = GetDirectIncidentCameraPosition(
            initialLookPoint,
            police,
            victim,
            ...
            if (playerView != null)
            {
                if (elapsed >= nextCameraRefresh)
                {
                    GetStableIncidentFrame(
                        ...
                        out dragVerticalExtent,
                        out dragFramingDistance);
                    dragCameraTarget = GetDirectIncidentCameraPosition(
                        dragLookTarget,
                        police,
                        victim,
                        dragPreferredDirection,
    ...
    private IEnumerator WatchFallbackRoutine()
    {
        ...
        HoldPoliceWitnessViewTowardsActors(draggingPolice, femaleVillagerActor);
        yield return PlayCinematicSpeakerLine(
            draggingPolice,
            "日警",
            "都給我安靜。你們最好記住自己的身分。",
        ...
        if (femaleVillagerActor != null && draggingPolice != null && hutEntrancePoint != null)
        {
            ...
            HoldPoliceWitnessViewTowardsPoint(
                hutEntrancePoint.position + Vector3.up * 1.2f);
    ...
    private IEnumerator MovePolicePairToExit(Transform first, Transform second, Vector3 target, float seconds)
    {
        ...
        if (fixedWitnessView != null)
        {
            Vector3 awayFromWitness = Vector3.ProjectOnPlane(
                pairStart - fixedWitnessPosition,
                Vector3.up);
            if (awayFromWitness.sqrMagnitude < 0.01f)
            {
                awayFromWitness = Vector3.ProjectOnPlane(
                    hasPoliceWitnessViewPose
                        ? policeWitnessViewRotation * Vector3.forward
                        : fixedWitnessView.forward,
                    Vector3.up);
            }
            if (awayFromWitness.sqrMagnitude < 0.01f)
            {
                awayFromWitness = Vector3.forward;
            }

            awayFromWitness.Normalize();
            float configuredDistance = Vector3.ProjectOnPlane(
                target - pairStart,
                Vector3.up).magnitude;
            float visibleExitDistance = Mathf.Max(configuredDistance, 24f);
            target = pairStart + awayFromWitness * visibleExitDistance;
        }
        ...
        {
            PlayPoliceWalkAnimation(first, 0f, policeFirstWalkAnimatorSpeed * 0.62f);
        }
        if (secondUsesAnimator)
        {
            PlayPoliceWalkAnimation(second, policeSecondWalkAnimationPhase, policeSecondWalkAnimatorSpeed * 0.62f);
        }
        ...
        endingCameraView = fixedWitnessView;
        ...
        if (endingCameraView != null)
        {
            HideIncidentShotOccluders(
                ...
                first,
                second,
                Mathf.Max(12f, exitActorHeight * 5f));
    ...
    private void ReleaseEndingCameraLock()
    {
        ...
        RestorePlayerHandRenderers(
            endingHiddenHandRenderers,
            endingHiddenHandRendererStates);
        endingHiddenHandRenderers = null;
        endingHiddenHandRendererStates = null;
    ...
    private void ReleaseIncidentCameraLock()
    {
        ...
        RestorePlayerHandRenderers(
            incidentHiddenHandRenderers,
            incidentHiddenHandRendererStates);
        incidentHiddenHandRenderers = null;
        incidentHiddenHandRendererStates = null;
    ...
    private void RepairInteractionHudRuntimeDefaults()
    {
        ...
        if (IsNewPoliceScene())
        {
            showSkipToIncidentButton = false;
            followPoliceBacksDuringExit = true;
            minimumPoliceExitSeconds = 8f;
            policeExitSeconds = 8f;
...
internal static class Chapter1WeddingRigBones
{
    public static Transform Resolve(
        Animator animator,
        HumanBodyBones humanoidBone,
        params string[] genericNames)
    {
        if (animator == null)
        {
            return null;
        ...
        Transform[] transforms =
            animator.transform.GetComponentsInChildren<Transform>(true);
        for (int nameIndex = 0; nameIndex < genericNames.Length; nameIndex++)
        {
            string expected = Normalize(genericNames[nameIndex]);
            for (int transformIndex = 0;
                transformIndex < transforms.Length;
                transformIndex++)
            {
                Transform candidate = transforms[transformIndex];
                if (candidate != null && Normalize(candidate.name) == expected)
                {
                    return candidate;
                }
            }
...
public sealed class Chapter1WeddingLimbDance : MonoBehaviour
{
    ...
    public Chapter1CircleDancer dancer;
    ...
    public void Configure(
        Animator sourceAnimator,
        Chapter1CircleDancer sourceDancer,
    ...
    private void LateUpdate()
    {
        ...
        Vector3 bodyRight = rightUpperArm.position - leftUpperArm.position;
        ...
        float beat = Time.time * beatsPerSecond * Mathf.PI * 2f
            + phaseRadians;
        float leftSwing = Mathf.Sin(beat) * armSwingDegrees;
        float rightSwing = Mathf.Sin(beat + Mathf.PI) * armSwingDegrees;
        float liftPulse = (0.7f + 0.3f * Mathf.Sin(beat * 0.5f))
            * armLiftDegrees;

        leftUpperArm.rotation = Quaternion.AngleAxis(leftSwing, bodyRight)
            * Quaternion.AngleAxis(-liftPulse, bodyForward)
            * leftUpperArm.rotation;
        rightUpperArm.rotation = Quaternion.AngleAxis(rightSwing, bodyRight)
            * Quaternion.AngleAxis(liftPulse, bodyForward)
            * rightUpperArm.rotation;
    }

    private bool HasSafeHumanoidRig()
    {
...
public sealed class Chapter1WeddingFaceCenter : MonoBehaviour
{
    ...
    private void CacheFacingBones()
    {
        ...
        leftShoulder = Chapter1WeddingRigBones.Resolve(
            animator,
            HumanBodyBones.LeftShoulder,
            "L_Shoulder",
            "LeftShoulder");
        rightShoulder = Chapter1WeddingRigBones.Resolve(
            animator,
    ...
    public void FaceCenterNow()
    {
        ...
        Vector3 visualForward;
        if (!TryGetSkeletonVisualForward(out visualForward))
        {
    ...
    private bool TryGetSkeletonVisualForward(out Vector3 visualForward)
    {
