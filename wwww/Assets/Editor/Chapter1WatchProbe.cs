...
{
    static readonly string Folder = Path.GetFullPath("_CodexBackups/silent_watch_20260925");
    ...
    {
        public bool playing, paused, compiling, completed, waiting, audioPlaying;
    ...
    static void Update()
    {
        ...
        if (EditorApplication.isPlaying && !EditorApplication.isPaused && controller != null)
        {
            ...
            {
                controller.stopPlayModeAfterChapterEnding = false;
                ...
                {
                    chosen = true;
                    if (mode == "watch") controller.ChooseWatch(); else controller.ChooseIntervene();
                }
            }
            string beat = Beat();
        ...
        switch (command)
        {
            ...
            case "play-watch": case "play-intervene": case "play-manual":
